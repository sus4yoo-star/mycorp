...
packages/
  ...
  api-client/         웹·모바일 공용 API 클라이언트 — **아직 아무도 안 쓴다** (아래)
...
`packages/api-client`는 **아직 연결되어 있지 않다.** 어떤 앱도 import하지 않고,
이 클라이언트가 호출하는 `/api/companies/…` 라우트는 존재하지 않는다. 웹은
`@mycorp24/db`로 Supabase에 직접 말하고, 모바일은 데이터 계층 자체가 없다.
스펙 §02가 요구하는 이음새라서 남겨두었을 뿐이다 — **타입이 붙어 있다고 해서
엔드포인트가 있는 것은 아니다.**

**모바일 앱은 표시 전용이다.** 세션도, API 클라이언트도, DB 접근도 없다.
그래서 공유 패키지로 계산할 수 있는 본사 층 구성만 보여주고, 나머지는
웹에서 보시라고 말한다. 한동안은 "오늘 직접 결정하셔야 할 일은 2건입니다"와
지어낸 결재 두 건을 띄우고 있었다 — 회장님이 가장 먼저 집는 화면에서 §151을
어기고 있었던 셈이다. 목업이라는 것은 변명이 되지 않는다. 화면 어디에도
그렇게 적혀 있지 않았다.
