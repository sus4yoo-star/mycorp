# MYCORP24 — 호칭 현지화 (Address & Title Localization)

앱 이름은 전 세계 공통 **`MYCORP24`** 하나다. 마스터 브랜드 `AMOV`도 번역하지 않는다.
...
1. **브랜드명은 번역하지 않는다.** 어느 지역에서도 `MYCORP24`다.
...
- [ ] 앱 이름은 어떤 로케일에서도 `MYCORP24` 로 표시된다
